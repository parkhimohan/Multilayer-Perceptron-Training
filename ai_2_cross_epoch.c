...
void errorBackPropagation(NEURALNET** neuralNetwork, int numOfLayers, int networkLayers[], int answer[]){
	...
	for(i=numOfLayers-1; i>0; i--){
		for(j=1; j<=networkLayers[i]; j++){
			if(i==(numOfLayers-1)) neuralNetwork[i][j].error = ((answer[j-1])-sigmoid(neuralNetwork[i][j].val));
			else{
				double temp = 0.0;
	...
	for(i=1; i<numOfLayers; i++){
		for(j=1; j<=networkLayers[i]; j++){
			for(k=0; k<=networkLayers[i-1]; k++){
				...
				else neuralNetwork[i][j].wts[k] += eta*
				
				sigmoid(neuralNetwork[i-1][k].val)*neuralNetwork[i][j].error;
			}
		}
	}
}
...
void plotGraph(int hidden[], double accuracyArray[]){
	char * commandsForGnuplot[] = {"set title \"CROSS ENTROPY WITH EPOCH\"", "set xlabel \"NODES IN HIDDEN LAYER\"" , "set ylabel \"ACCURACY\"", "plot 'data.temp' with linespoints title 'ACCURACY'"};
