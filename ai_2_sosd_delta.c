	...
	for(i=1; i<numOfLayers; i++){
		for(j=1; j<=networkLayers[i]; j++){
			for(k=0; k<=networkLayers[i-1]; k++){
				...
				neuralNetwork[i][j].wts[k] += eta*sigmoid(neuralNetwork[i-1][k].val)*neuralNetwork[i][j].error;
			delta += eta*sigmoid(neuralNetwork[i-1][k].val)*neuralNetwork[i][j].error;
				}
			}
		}
	}
...
void plotGraph(int hidden[], double accuracyArray[]){
	char * commandsForGnuplot[] = {"set title \"SUM OF SQUARED DEVIATION LOSS WITH DELTA\"", "set xlabel \"NODES IN HIDDEN LAYER\"" , "set ylabel \"ACCURACY\"", "plot 'data.temp' with linespoints title 'ACCURACY'"};
